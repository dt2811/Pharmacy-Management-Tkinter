...
import os
import random
conn = sqlite3.connect("C:\\Users\dhrum\Desktop\Pharmacy Management\Database\store.db") 
c = conn.cursor()
...
class Application:
    def __init__(self, master, *args, **kwargs):
        ...
        self.master = master
        #frames
        self.left = Frame(master, width=710, height=768, bg='white')
        self.left.pack(side=LEFT)

        self.right = Frame(master, width=656, height=768, bg='lightblue')
        self.right.pack(side=RIGHT)

        #components
        self.heading = Label(self.left, text="Neo Pharmacy Store", font=('arial 40 bold'), bg='white')
        self.heading.place(x=0, y=0)

        self.date_l = Label(self.right, text="Date: " + str(date), font=('arial 16 bold'), bg='lightblue', fg='white')
        ...
        self.enteride = Entry(self.left, width=25, font=('arial 18'), bg='lightblue')
        ...
        self.search_btn.place(x=380, y=140)

        #to be filled by ajax function
        ...
        #total label
        self.total_l = Label(self.right, text="", font=('arial 25 bold'), bg='lightblue', fg='white')
    ...
    def ajax(self, *args, **kwargs):
        ...
        self.quantity_e = Entry(self.left, width=25, font=('arial 18'), bg='lightblue')
        ...
        self.discount_e = Entry(self.left, width=25, font=('arial 18'), bg='lightblue')
        ...
        self.add_to_cart_btn.place(x=380, y=450)
        ...
        self.change_e = Entry(self.left, width=25, font=('arial 18'), bg='lightblue')
        self.change_e.place(x=220, y=500)

        #change button
        self.change_btn = Button(self.left, text="Calculate Change", width=22, height=2, bg='orange', command=self.change_func)
        self.change_btn.place(x=380, y=550)

        #generate bill button
        self.bill_btn = Button(self.left, text="Generate Bill", width=40, height=2, bg='red', fg='white', command=self.generate_bill)
        self.bill_btn.place(x=250, y=600)

    def add_to_cart(self, *args, **kwargs):
        #get the quantity value and add* to the database
        self.quantity_value = int(self.quantity_e.get())
        if self.quantity_value > int(self.get_stock):
            tkinter.messagebox.showinfo("Error", "Quantity exceeds maximum amount available.")
        else:
            #calculate the price
            ...
            for self.p in products_list:
                ...
                self.tempname.place(x=self.x_index, y=self.y_index)
    ...
    def change_func(self, *args, **kwargs):
        #get the amount given by the customer and total generated by computer
        ...
        self.c_amount.place(x=0, y=550)

    def generate_bill(self, *args, **kwargs):
        #generate the bill before updating to the database
        directory = "C:\\Users\dhrum\Desktop\Pharmacy Management\Invoice" + str(date) + "/"
        if not os.path.exists(directory):
            os.makedirs(directory)

        #TEMPLATES FOR THE BILL
        company = "\t\t\t\tNeo Pharmacy Store Ltd.\n"
        ...
        self.enteride.focus()
        tkinter.messagebox.showinfo("Success", "Purchase Completed Successfully.")
